...
float frequencies[] = {c4, db4, d4, eb4, e4, f4, gb4, g4, ab4, a4, bb4, b4, c5};
float PWM[13];
...
void loop() {
  ...
  // Determine if one or both FSRs are pressed
  ...
  // If the first FSR has been pressed
  if (pressed1 == true) {
    // Play the note C4 in accordance to its intensity
    analogWrite(speaker, intensity1);
    delayMicroseconds(PWM[0]);
    analogWrite(speaker, 0);
    delayMicroseconds(PWM[0]);
  }
  ...
  // If the second FSR has been pressed
  if (pressed2 == true) {
    // Play the note C5 in accordance to its intensity
    analogWrite(speaker, intensity2);
    delayMicroseconds(PWM[12]);
    analogWrite(speaker, 0);
    delayMicroseconds(PWM[0]);
  }
